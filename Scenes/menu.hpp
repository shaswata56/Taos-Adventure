...
Sprite backgroundSprite, backgroundSprite2, 
       normalSprite, soundOffSprrite, musicOffSprite;
...
void menu(RenderWindow &app){
    ...
    if(Mouse::isButtonPressed(Mouse::Left)){
        ...
        if(mousePosition.x >= 545 && mousePosition.x <= 690 
	   && mousePosition.y >= 400 && mousePosition.y <= 550)
        {
            currentScene = startingAnime;
        }
        if(mousePosition.y >= 605 && mousePosition.y <= 710)
	{
            if(mousePosition.x >= 435 && mousePosition.x <= 540 
	       && menuButtonClock.getElapsedTime().asMilliseconds()>500)
	    {
                if(musicOn){
                    musicOn = !musicOn;
                    mainbg.pause();
	    ...
	    if(mousePosition.x >= 650 && mousePosition.x <= 745 
	       && menuButtonClock.getElapsedTime().asMilliseconds()>500)
	    {
                soundOn = !soundOn;
                menuButtonClock.restart();
        ...
        if(mousePosition.x >= 1076 && mousePosition.x <= 1169 
	   && mousePosition.y >= 30 && mousePosition.y <= 121)
            app.close();
    }
    ...
    if(backgroundSprite.getPosition().x<-1200) 
	    backgroundSprite.setPosition(1200, 0);
    if(backgroundSprite2.getPosition().x<-1200) 
	    backgroundSprite2.setPosition(1200, 0);
    app.draw(backgroundSprite);
    app.draw(backgroundSprite2);
    app.draw(normalSprite);
    if(!musicOn) 
	    app.draw(musicOffSprite);
    if(!soundOn) 
	    app.draw(soundOffSprrite);
}
